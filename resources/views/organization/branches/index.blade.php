                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left font-medium text-gray-500">Kota</th>
                            <th class="px-5 py-3 text-right font-medium text-gray-500">Divisi</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($branches as $branch)
                            <tr>
                                <td class="px-5 py-4">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ $branch->code }}</p>
                                </td>
                                <td class="px-5 py-4 text-gray-600">{{ $branch->type === 'warehouse' ? 'Gudang' : 'Office' }}</td>
                                <td class="px-5 py-4 text-gray-600">{{ $branch->city ?? '-' }}</td>
                                <td class="px-5 py-4 text-right text-gray-600">{{ number_format($branch->departments_count) }}</td>
                                <td class="px-5 py-4 text-right text-gray-600">{{ number_format($branch->active_employees_count) }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        @can('organization.update')
                                            <a href="{{ route('organization.branches.edit', $branch) }}" class="rounded-md border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50">Edit</a>
                                        @endcan
                        ...
                        @empty
                            <tr><td colspan="6" class="px-5 py-10 text-center text-gray-500">Belum ada lokasi kerja.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

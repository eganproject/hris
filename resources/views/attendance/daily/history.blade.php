<x-layouts.app title="Riwayat Absensi - {{ $employee->full_name }}" heading="Riwayat Absensi">
    <div class="mx-auto max-w-5xl space-y-6">
        <section class="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
                <p class="text-sm font-medium text-gray-500"><a href="{{ route('attendance.daily.index', ['branch_id' => $branchId]) }}" class="hover:text-gray-700">‹ Absensi Harian</a> · {{ $month->translatedFormat('F Y') }}</p>
                <h1 class="mt-1 text-2xl font-semibold text-gray-950">{{ $employee->full_name }}</h1>
                <p class="mt-1 text-sm text-gray-500">
                    {{ $employee->employee_number ?? '—' }}
                    · {{ $employee->jobPosition?->name ?? '—' }}
                    · {{ $employee->departments->pluck('name')->filter()->join(', ') ?: '—' }}
                    · {{ $employee->branch?->name ?? '—' }}
                </p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('attendance.daily.history', ['employee' => $employee, 'month' => $prevMonth, 'branch_id' => $branchId]) }}" class="rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50" aria-label="Bulan sebelumnya">‹</a>
                <form method="GET" action="{{ route('attendance.daily.history', $employee) }}">
                ...
                <a href="{{ route('attendance.daily.history', ['employee' => $employee, 'month' => $nextMonth, 'branch_id' => $branchId]) }}" class="rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50" aria-label="Bulan berikutnya">›</a>
            </div>
        </section>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse ($records as $record)
                            @php $isWeekend = in_array($record->work_date->dayOfWeek, [0, 6], true); @endphp
                            <tr class="{{ $isWeekend ? 'bg-gray-50/60' : '' }}">
                                ...
                                <td class="text-center text-sm {{ $record->late_minutes > 0 ? 'font-medium text-amber-600' : 'text-gray-700' }}">{{ $record->clock_in_label }}</td>

                    ...
                    @if ($employees->total() > 0)
                        <div class="flex flex-col gap-3 border-t border-gray-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-center gap-3">
                                <form method="GET" action="{{ route('attendance.leave-balances.index') }}" class="flex items-center gap-2">
                                    @foreach (request()->except(['per_page', 'page']) as $key => $value)
                                        <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                    @endforeach
                                    <label for="balance_per_page" class="text-xs text-gray-500">Per halaman</label>
                                    <select id="balance_per_page" name="per_page" onchange="this.form.submit()" class="rounded-md border border-gray-300 px-2 py-1.5 text-xs shadow-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/20">
